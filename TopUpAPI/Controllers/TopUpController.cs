...
namespace TopUpAPI.Controllers
{
    ...
    public class TopUpController : BaseController
    {
        ...
        public async Task<ActionResult<List<int>>> GetTopUpOptions()
        {
            try
            {
                var topUpOptions = await _topUpService.GetTopUpOptions();
        ...
        public async Task<ActionResult> TopUpBeneficiary([FromBody] TopUpRequest request)
        {
            if (ModelState.IsValid)
            {
                ...
                var IsBalanceSufficient = await _topUpService.IsBalanceSufficient(userId, request.Amount);
